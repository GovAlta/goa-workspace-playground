...
export function ClientsPage() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof Client | '', direction: 'asc' | 'desc' | 'none' }>({ key: '', direction: 'none' });
      ...
      <div className="clients-filter-section">
        ...
        <GoabFormItem id="filterInput" error={inputError}>
          <GoabBlock gap="xs" direction="row" alignment="start">
            ...
            />
            <GoabButton type="secondary" onClick={() => applyFilter()} leadingIcon="filter" size={isMobile ? "medium" : "large"}>
              Filter
            </GoabButton>
          </GoabBlock>
        </GoabFormItem>
            ...
            <thead>
              <tr data-grid="row">
                <th data-grid="cell" style={{ paddingBottom: 0 }}>
                  <GoabCheckbox name="selectAll" value={allSelected} onChange={() => setAllSelected(!allSelected)} ariaLabel="Select all clients" />
                </th>
